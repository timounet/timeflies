...
## Bootstrapping the Project REST Endpoint
Created with maven archetype template:
`````shell script
mvn io.quarkus:quarkus-maven-plugin:1.4.2.Final:create \
    -DprojectGroupId=org.timeflies \
    -DprojectArtifactId=rest-project \
    -DclassName="org.timeflies.projects.ProjectResource" \
    -Dpath="api/projects" \
...
The Maven archetype generates the following rest-project sub-directory:
- the Maven structure with a pom.xml
- an org.timeflies.projects.ProjectResource resource exposed on /api/projects
- an associated unit test ProjectResourceTest
- the landing page index.html that is accessible on [http://localhost:8084](http://localhost:8084) after starting the application
...
hello
````
Alternatively, you can open [http://localhost:8084/api/projects](http://localhost:8084/api/projects) in your browser

## Transactions and ORM
   
The Project API’s role is to allow CRUD operations on timeflies projects. In this module we create a Project entity and persist/update/delete/retrieve it from a Postgres database in a transactional way.
...
proj.name = "Superman";
proj.description = "plop";
...
// persist it
proj.persist();

// getting a list of all Projects entities
...
````properties
quarkus.datasource.url=jdbc:postgresql://localhost:5432/projects_database
quarkus.datasource.driver=org.postgresql.Driver
quarkus.datasource.username=chief
quarkus.datasource.password=chief
quarkus.datasource.max-size=8
quarkus.datasource.min-size=2
````
### Transactional Service 

To manipulate the Projects entity we will develop a transactional ProjectService class. The idea is to wrap methods modifying the database (e.g. entity.persist()) within a transaction. Marking a CDI bean method @Transactional will do that for you and make that method a transaction boundary.
...
Notice that both methods that persist and update a Project, pass a Projects object as a parameter. Thanks to the Bean Validation’s @Valid annotation, the Project object will be checked to see if it’s valid or not. If it’s not, the transaction will be rollback-ed.
...
````
A [DatabaseResource test class](./src/test/java/org/timeflies/projects/DatabaseResource.java) starts the database before the application, and set the quarkus.datasource.url system property to indicate to the application where is the test database. Note that the database would use a random port.
...
## OpenAPI
Now, you curl http://localhost:8084/openapi endpoint
...
## Swagger UI 
When building APIs, developers want to test them quickly. Swagger UI is a great tool permitting to visualize and interact with APIs. The UI is automatically generated from your OpenAPI specification. The Quarkus smallrye-openapi extension comes with a swagger-ui extension embedding a properly configured Swagger UI page. By default, Swagger UI is accessible at /swagger-ui. So, once your application is started, you can go to http://localhost:8084/swagger-ui and play with API.
...
- [/health/live](http://localhost:8084/health/live) - The application is up and running.
- [/health/ready](http://localhost:8084/health/ready) - The application is ready to serve requests.
- [/health](http://localhost:8084/health) - Accumulating all health check procedures in the application.
...
- Custom Liveness Healthcheck : [PingProjectResourceHealthCheck](src/main/java/org/timeflies/projects/health/PingProjectResourceHealthCheck.java) 
...
Adding Metrics to ProjectResource:
...
- countGetXXX: A counter which is increased by one each time the project gets the XXX method.
...
````shell script
$ curl -H "Accept: application/json" http://localhost:8084/metrics/application
````
...
[Docker Hub timeflies Project API](https://hub.docker.com/repository/docker/timefliesapp/projectss-api)
![projects api](../doc/docker-hub-rest-projects.png)
...
````shell script
$ docker run --rm --name tfproject -p 8084:8084 timefliesapp/projects-api:1.0-SNAPSHOT
# In previous case the container would not reach the database
$ docker run --rm --name tfprojects --net="host" timefliesapp/projects-api:1.0-SNAPSHOT
# In this case, it should join the dev database but it s unesecured
````
To remove container:
````shell script
$ docker run kill tfprojects
# Check containers
$ docker ps
$ docker stats
````
